...
def prepare(X: pd.DataFrame) -> pd.DataFrame:
    ...
    df['Mean_Elevation_Vertical_Distance_Hydrology'] = df[['Elevation', 
                                                    'Vertical_Distance_To_Hydrology']].mean(axis=1)
    ...
    df['Mean_Distance_Firepoints_Roadways'] = df[['Horizontal_Distance_To_Fire_Points', 
                                                'Horizontal_Distance_To_Roadways']].mean(axis=1)
    df['sqrt_Horizontal_Distance_To_Hydrology'] = np.sqrt(df["Horizontal_Distance_To_Hydrology"])
    ...
    remove_base_features = ["Id", "Horizontal_Distance_To_Hydrology",
                    "Elevation", "Aspect",
                    "Hillshade_3pm",
