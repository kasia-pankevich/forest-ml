...
@click.command()
@click.option("-m", "--model-path", default="data/model.joblib", 
            type=click.Path(exists=True, dir_okay=False, path_type=Path),
            help="File path to saved model .joblib file")
@click.option("-t", "--test-data-path", default="data/test.csv",
            type=click.Path(exists=True, dir_okay=False, path_type=Path),
            help="File path to test data .csv file")
@click.option("-s", "--submission-path", default="data/submission.csv",
...
def predict(model_path: Path, test_data_path: Path, submission_path: Path) -> None:
    ...
    pd.DataFrame({"Id": X_test_in["Id"], "Cover_Type": y_predicted}).set_index(["Id"]).to_csv(submission_path)